...
pub async fn run(cli: Cli) -> Result<(), Box<dyn std::error::Error>> {
    match cli.command {
        ...
        Commands::Triage { target } => run_triage(&target).await,
        Commands::Status => run_status().await,
        Commands::Cleanup { target } => run_cleanup(&target).await,
    }
}
...
async fn run_triage(target: &str) -> Result<(), Box<dyn std::error::Error>> {
    let (owner, (repo, number)) = split_target(target)?;
    let db_path = std::env::var("MONKEY_DB_PATH").unwrap_or_else(|_| "/data/monkey.db".to_string());
    let store = Store::new(&db_path)?;

    match store.latest_event_for_issue(&owner, &repo, number).await? {
        Some(event) => {
...
async fn run_cleanup(target: &str) -> Result<(), Box<dyn std::error::Error>> {
    let (owner, (repo, number)) = split_target(target)?;
    let workspaces_root =
        std::env::var("MONKEY_WORKSPACES_ROOT").unwrap_or_else(|_| "/data/workspaces".to_string());
    cleanup_workspace(Path::new(&workspaces_root), &owner, &repo, number).await?;
...
pub enum TargetError {
    #[error("missing issue number after '#'")]
    MissingNumber,
    #[error("invalid issue number: {0}")]
    InvalidNumber(#[from] std::num::ParseIntError),
    #[error("missing repo, expected owner/repo")]
    MissingRepo,
}

pub fn split_target(target: &str) -> Result<(String, (String, i64)), TargetError> {
    let (repo_part, number_part) = target.split_once('#').ok_or(TargetError::MissingNumber)?;

    let number: i64 = number_part.parse()?;

    let (owner, repo) = repo_part.split_once('/').ok_or(TargetError::MissingRepo)?;
